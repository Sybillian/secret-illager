...
execute if entity @a[tag=pres,tag=!dead] run tellraw @a [{"translate":"syb.chat.new_pres","with":[{"translate":"syb.prefix.game","color":"green","bold":"true"},{"selector":"@a[tag=pres]"}]}]
execute if entity @a[tag=pres,tag=!dead] run schedule function syb:election/trigger/nom_chancellor 2s

execute if entity @a[tag=pres,tag=dead] run function syb:election/trigger/new_pres
