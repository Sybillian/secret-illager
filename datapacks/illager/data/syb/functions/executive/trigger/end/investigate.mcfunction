...
execute as @a[tag=pres] if score @a[tag=investigate_target] role matches 1 run tellraw @s {"translate":"syb.chat.investigate.selected.pres","with":[{"selector":"@a[tag=investigate_target]"},{"translate":"syb.generic.villager","color":"green"}]}
execute as @a[tag=pres] if score @a[tag=investigate_target] role matches 2.. run tellraw @s {"translate":"syb.chat.investigate.selected.pres","with":[{"selector":"@a[tag=investigate_target]"},{"translate":"syb.generic.illager","color":"red"}]}
